...
static ssize_t _polytech_handler(coap_pkt_t *pkt, uint8_t *buf, size_t len, void *context)
{
    (void)context;
    
    char * str = "Les cours a distance, c'est genial";
    
    return coap_reply_simple(pkt, COAP_CODE_205, buf, len,
            COAP_FORMAT_TEXT, (uint8_t*)str, strlen(str));
}

static ssize_t _dublin_handler(coap_pkt_t *pkt, uint8_t *buf, size_t len, void *context)
{
    (void)context;
...
const coap_resource_t coap_resources[] = {
    ...
    { "/dublin", COAP_GET, _dublin_handler, NULL },
    { "/echo/", COAP_GET | COAP_MATCH_SUBTREE, _echo_handler, NULL },
    { "/polytech", COAP_GET, _polytech_handler, NULL },
